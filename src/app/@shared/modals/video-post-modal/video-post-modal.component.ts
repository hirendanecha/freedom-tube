...
export class VideoPostModalComponent {
  ...
  uploadImgAndSubmit(): void {
    if (this.postData?.profileid) {
      let uploadObs = {};
      if (this.postData?.file1?.name) {
        uploadObs['streamname'] = this.commonService.upload(this.postData?.file1);
      }

      if (this.postData?.file2?.name) {
        uploadObs['thumbfilename'] = this.commonService.upload(this.postData?.file2);
      }
